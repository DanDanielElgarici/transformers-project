...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        if self.arch == 'trans_enc':
            print("DiT TRANS_ENC init with In-Context Conditioning")
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.arch == 'trans_enc':
            ...
            # Process through standard transformer encoder (conditioning tokens are part of the sequence)
            # This implements DiT In-Context Conditioning where timestep + text are treated as additional tokens
            full_output = self.seqTransEncoder(xseq, src_key_padding_mask=frames_mask)
            
            # Remove conditioning tokens from the output (keep only motion tokens)
            # In-context conditioning: conditioning tokens are processed but removed from final output
            output = full_output[1:]  # [seqlen, bs, d] - remove first token (conditioning)
