...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        self.input_process = InputProcess(self.data_rep, self.nfeats, self.latent_dim)
        ...
        if self.arch == 'trans_enc':
            ...
            self.dit_blocks = nn.ModuleList([
                DiTBlockWithAdaLNZero(
                    ...
                    hop_k=1,  # start with 1-hop neighbors
                    adj_matrix=None  # or provide a [J,J] bool here
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.arch == 'trans_enc':
            ...
            # --- NEW: infer T,J and prepare adjacency ---
            T = nframes  # number of frames
            J = njoints  # number of joints

            # Optional adjacency sources (pick whatever you have):
            # 1) from y (per-batch), 2) from a model attribute, else None (falls back to chain)
            adj_matrix = None
            if isinstance(y, dict) and ('adj' in y or 'adj_matrix' in y):
                adj_matrix = y.get('adj', y.get('adj_matrix'))  # expect [J, J] bool/0-1 tensor on the right device
            elif hasattr(self, 'adj_matrix') and self.adj_matrix is not None:
                adj_matrix = self.adj_matrix

            # (Sanity) If your tokenization doesn’t produce S=T*J, you’ll see this assert.
            # That tells you your input_process packs tokens differently.
            # In that case, either adapt input_process or switch to a spatial-only mask.
            assert motion_tokens.shape[0] == T * J, \
                f"Neighborhood attention expects S=T*J; got S={motion_tokens.shape[0]}, T={T}, J={J}"

            # --- Call DiT blocks with neighborhood attention (temporal=global, spatial k-hop) ---
            for dit_block in self.dit_blocks:
                motion_tokens = dit_block(
                    motion_tokens,
                    cond_token.squeeze(0),  # [bs, d]
                    T=T,
                    J=J,
...
class InputProcess(nn.Module):
    def __init__(self, data_rep, nfeats, latent_dim):
        super().__init__()
        self.data_rep = data_rep
        self.nfeats = nfeats
        self.latent_dim = latent_dim
        ...
        if self.data_rep == 'rot_vel':
            self.velEmbedding = nn.Linear(self.nfeats, self.latent_dim)  # per-joint variant if you ever use it

# forward
    def forward(self, x):
        # x: [B, J, F, T]
        bs, njoints, nfeats, nframes = x.shape
        x = x.permute(3, 0, 1, 2).contiguous()          # [T, B, J, F]
        x = x.view(nframes * bs * njoints, nfeats)      # [T*B*J, F]
        x = self.poseEmbedding(x)                       # [T*B*J, d]
        x = x.view(nframes * njoints, bs, self.latent_dim)  # [S=T*J, B, d]
        return x
...
class OutputProcess(nn.Module):
    ...
    def forward(self, output):
        ...
        if self.data_rep in ['rot6d', 'xyz', 'hml_vec']:
            output = self.poseFinal(output)                # [T*J, B, F]
            output = output.view(T, J, bs, self.nfeats)    # [T, J, B, F]
            output = output.permute(2, 1, 3, 0).contiguous()  # [B, J, F, T]
        elif self.data_rep == 'rot_vel':
            raise NotImplementedError("Add the per-joint variant if you use rot_vel")
        else:
            raise ValueError
        return output
...
class DiTBlockWithAdaLNZero(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, dropout=0.1, activation="gelu", hop_k=1, adj_matrix=None):
        super().__init__()

        # Multi-head self-attention
        self.attn = NeighborhoodSelfAttention(
        ...
        self.register_buffer("adj_matrix", None, persistent=False)
        if adj_matrix is not None:
            self.register_buffer("adj_matrix", adj_matrix.bool(), persistent=False)
    ...
    def forward(self, x, c, *, T: int, J: int, adj_matrix: torch.Tensor | None = None):
        """
        ...
        residual = x
        x_norm = self.norm1(x)
        x_mod = x_norm * (1 + scale_msa.unsqueeze(0)) + shift_msa.unsqueeze(0)
        attn_out = self.attn(x_mod, T=T, J=J, adj_matrix=adj_matrix)
...
def _k_hop_adjacency(adj: torch.Tensor, k: int, include_self: bool = True) -> torch.Tensor:
    """
    Compute boolean reachability up to k hops.
    adj: [J,J] bool/0-1
    return: [J,J] bool
    """
    J = adj.shape[0]
    A = adj.to(dtype=torch.float32)
    reach = torch.zeros_like(A)
    P = A.clone()
    for _ in range(k):
        reach = reach + P
        P = P @ A
    if include_self:
        eye = torch.eye(J, device=adj.device, dtype=torch.bool)
        return (reach > 0).to(torch.bool) | eye
    return (reach > 0).to(torch.bool)
...
class NeighborhoodSelfAttention(nn.Module):
    ...
    @staticmethod
    def _default_chain_adj(J: int, device):
        adj = torch.zeros((J, J), device=device, dtype=torch.bool)
        idx = torch.arange(J, device=device)
        adj[idx[1:], idx[:-1]] = True
        adj[idx[:-1], idx[1:]] = True
        return adj

    def _build_spatial_mask(self, J: int, device, adj_matrix: torch.Tensor | None):
        if adj_matrix is None:
            adj = self._default_chain_adj(J, device)
        else:
            adj = adj_matrix.to(device=device, dtype=torch.bool)
        reach = _k_hop_adjacency(adj, self.hop_k, include_self=self.include_self)  # [J,J]
        mask = torch.zeros((J, J), device=device, dtype=torch.float32)
        mask[~reach] = float('-inf')  # additive mask
        return mask

    def forward(self, x: torch.Tensor, *, T: int, J: int, adj_matrix: torch.Tensor | None = None):
        S, B, D = x.shape
        assert S == T * J, f"Expected S=T*J, got S={S}, T={T}, J={J}"

        # ---- Temporal global attention (per joint) ----
        # reshape to [T, B*J, D]
        x_t = x.view(T, J, B, D).permute(0, 2, 1, 3).contiguous().view(T, B * J, D)
        y_t, _ = self.temporal_mha(x_t, x_t, x_t, attn_mask=None)  # full attention
        y = y_t.view(T, B, J, D).permute(0, 2, 1, 3).contiguous().view(S, B, D)

        # ---- Spatial neighborhood attention (per frame) ----
        # reshape to [J, B*T, D]
        z = y.view(T, J, B, D).permute(1, 2, 0, 3).contiguous().view(J, B * T, D)
        spatial_mask = self._build_spatial_mask(J, y.device, adj_matrix)  # [J,J]
        z_out, _ = self.spatial_mha(z, z, z, attn_mask=spatial_mask)
        out = z_out.view(J, B, T, D).permute(2, 0, 1, 3).contiguous().view(S, B, D)
        return out