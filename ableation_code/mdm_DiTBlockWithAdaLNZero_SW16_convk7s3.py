# make shore kargs have the 'attn_window' key, try 8, 12, 16
...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        self.tokenization = kargs.get('tokenization', 'conv_temporal')
        self.conv_kernel = kargs.get('conv_kernel', 7)
        self.conv_stride = kargs.get('conv_stride', 3)
        self.conv_padding = kargs.get('conv_padding', 'same')
        ...
        if self.tokenization == 'conv_temporal':
            self.tokenizer = ConvTemporalTokenizer(self.data_rep, self.njoints, self.nfeats, self.latent_dim,
                                                   kernel_size=self.conv_kernel, stride=self.conv_stride,
                                                   padding=self.conv_padding)
            self.detokenizer = ConvTemporalDetokenizer(self.data_rep, self.njoints, self.nfeats, self.latent_dim,
                                                       kernel_size=self.conv_kernel, stride=self.conv_stride,
                                                       padding=self.conv_padding)

        if self.arch == 'trans_enc':
            print("DiT TRANS_ENC init with adaLN-Zero")
            attn_window = kargs.get('attn_window', 8)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.tokenization == 'conv_temporal':
            tokens, tok_aux = self.tokenizer(x)
            x = tokens
        else:
        ...
        if self.mask_frames and is_valid_mask and self.tokenization == 'conv_temporal':
            bs, _, _, T = y['mask'].shape
            S = tok_aux["S"]
            token_centers = torch.linspace(0, T - 1, steps=S, device=x.device)
            idx = token_centers.round().long().clamp_(0, T - 1)
            fm = torch.logical_not(y['mask'][..., idx].squeeze(1).squeeze(1)).to(x.device)   # [bs, S]
            frames_mask = torch.cat([torch.zeros((bs, 1), dtype=torch.bool, device=x.device), fm], dim=1) \
                if self.arch == 'trans_enc' else fm
        ...
        if self.is_prefix_comp:
            if self.tokenization == 'conv_temporal':
                # token-aware trim (see #2 below)  <-- keep this block in sync with #2
                T = tok_aux["T"];
                S = tok_aux["S"]
                # map tokens to source-frame centers
                token_centers = torch.linspace(0, T - 1, steps=S, device=output.device)
                keep = token_centers >= (self.context_len - 1e-6)
                first_keep = int(torch.nonzero(keep, as_tuple=False)[0].item()) if keep.any() else S
                output = output[first_keep:]
            else:
        ...
        if self.tokenization == 'conv_temporal':
            ...
            xrec = self.output_process(output)  # original path
...
class DiTBlockWithAdaLNZero(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, dropout=0.1, activation="gelu", window_size=8):
        super().__init__()
...
class SlidingWindowSelfAttention(nn.Module):
    def __init__(self, hidden_size, num_heads, dropout=0.0, window_size=12): #Change here the window size
        super().__init__()
...
class ConvTemporalTokenizer(nn.Module):
    """
    Depthwise-agnostic (dense) Conv1d over time on flattened features -> tokens.
    """
    def __init__(self, data_rep, njoints, nfeats, latent_dim, kernel_size=5, stride=2, padding="same"):
        super().__init__()
        ...
        self.conv = nn.Conv1d(in_channels=self.input_feats,
    ...
    def forward(self, x):
        """
        x: [bs, J, F, T]
        returns tokens: [S, bs, D], aux
        """
        bs, J, F, T = x.shape
        x = x.view(bs, J*F, T)            # [bs, C_in, T]
        y = self.conv(x)                  # [bs, D, S]
        S = y.shape[-1]
        tokens = y.permute(2,0,1).contiguous()  # [S, bs, D]
        return tokens, {"T": T, "S": S}

class ConvTemporalDetokenizer(nn.Module):
    """
    TransposedConv to upsample tokens back to frame resolution.
    """
    def __init__(self, data_rep, njoints, nfeats, latent_dim, kernel_size=5, stride=2, padding="same"):
        super().__init__()
        self.data_rep = data_rep
        ...
        self.deconv = nn.ConvTranspose1d(in_channels=self.latent_dim,
                                         out_channels=self.input_feats,
                                         kernel_size=self.kernel_size,
                                         stride=self.stride,
                                         padding=pad,
                                         output_padding=0)
    ...
    def forward(self, y_tokens, aux):
        ...
        returns: [bs, J, F, T]
        """
        S, bs, D = y_tokens.shape
        T_target = aux["T"]
        y = y_tokens.permute(1,2,0).contiguous()  # [bs, D, S]
        x = self.deconv(y)                        # [bs, C_in, T_hat]
        # center-crop or pad to exact T_target
        T_hat = x.shape[-1]
        if T_hat > T_target:
            x = x[..., :T_target]
        elif T_hat < T_target:
            pad = T_target - T_hat
            x = F.pad(x, (0, pad))
        x = x.view(bs, self.njoints, self.nfeats, T_target)
        return x
