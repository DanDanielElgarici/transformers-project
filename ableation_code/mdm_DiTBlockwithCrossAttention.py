...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        if self.arch == 'trans_enc':
            print("DiT TRANS_ENC init with Cross-Attention")
            # Create DiT blocks with Cross-Attention
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.arch == 'trans_enc':
            ...
            for dit_block in self.dit_blocks:
                motion_tokens = dit_block(motion_tokens, cond_token)
...
class DiTBlockWithCrossAttention(nn.Module):
    """
    DiT block with cross-attention conditioning.
    Adapted from Facebook Research DiT implementation for MDM.
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, dropout=0.1):
        ...
        # Multi-head cross-attention (NEW - this is the key difference)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.ReLU(),  # Using ReLU for better compatibility with older PyTorch versions
            nn.Dropout(dropout),
            nn.Linear(mlp_hidden_dim, hidden_size),
            nn.Dropout(dropout)
        )
        
        # Conditioning embedding (for timestep + text)
    ...
    def forward(self, x, cond):
        ...
        # Cross-attention with conditioning (NEW)
...
class AdaptiveLayerNorm(nn.Module):
    ...
    def __init__(self, d_model):
        ...
        # MLP to regress scale, shift, and alpha parameters
        self.adaLN_mlp = nn.Sequential(
            nn.ReLU(),  # Using ReLU for better compatibility with older PyTorch versions
            nn.Linear(d_model, 6 * d_model)  # 4 for adaLN + 2 for alpha
        )
        
        # Initialize alpha to zero for adaLN-Zero
        nn.init.constant_(self.adaLN_mlp[-1].weight, 0)
        nn.init.constant_(self.adaLN_mlp[-1].bias, 0)
    
    def forward(self, x, c):
        # c is the conditioning (timestep + text embedding)
        adaLN_params = self.adaLN_mlp(c)
        
        # Split parameters: [shift_msa, scale_msa, shift_mlp, scale_mlp, alpha1, alpha2]
        shift_msa, scale_msa, shift_mlp, scale_mlp, alpha1, alpha2 = adaLN_params.chunk(6, dim=1)
        
        return shift_msa, scale_msa, shift_mlp, scale_mlp, alpha1, alpha2


def modulate(x, shift, scale):
    """
    Modulate the input with shift and scale parameters.
    """
